...
float readCurrent(int pin, float zeroV) {
  ...
  float current = (voltage - zeroV) / 0.185;
  if (fabs(current) < 0.1) current = 0;
  return current;
}

float getRandomVoltage() {
  return 10.0 + (random(0, 2001) / 1000.0); // 10.000 to 12.000
}
...
void loop() {
  ...
  if (currentMillis - prevMillis >= interval) {
    ...
    float voltage = getRandomVoltage();
    ...
    if (Firebase.ready() && signupOK) {
      ...
      if (Firebase.RTDB.setFloat(&fbdo, "/voltage", voltage)) {
        ...
        Serial.printf("❌ Firebase error (Voltage): %s\n", fbdo.errorReason().c_str());
      }
    }
  }
}
